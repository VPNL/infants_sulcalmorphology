...
%    brain scan data, so the code is included for reference only.

% 2. RUNNABLE CODE: Creates Figure 6b using csv data.
...
%% RUNNABLE CODE 
% Here, I'm running the sts (8th roi in roi_list) 
% for the 24th subject in our dataset
...
for r = 8
    ...
    % Paths to change
    SD_profiles = readtable(['profiles_SD_' roi_name '.csv']);
    R1_profiles = readtable(['profiles_R1_' roi_name '.csv']);
    CT_profiles = readtable(['profiles_CT_' roi_name '.csv']);
    CU_profiles = readtable(['profiles_CU_' roi_name '.csv']);
    ...
    for i = 24
      ...
      [r1, p] = corrcoef(CU_profile, SD_profile, 'rows', 'complete');
       title([char(subj(i)) ' - R: ', num2str(r1(1,2), '%.2f'), ' p: ', num2str(p(1,2))])     
   
        if i == 1
            legend('SD', 'CU', 'Location', 'best');
        end
        hold off;
    end
    ...
    for i = 24
        ...
        [r1, p] = corrcoef(R1_profile, SD_profile, 'rows', 'complete');
        title([num2str(regage(i))  ' days - R: ', num2str(r1(1,2), '%.2f'), ' p: ', num2str(p(1,2))]);

        if i == 1
            legend('SD', 'R1', 'Location', 'best');
        end
        hold off;
    end
    ...
    for i = 24
        ...
        [r1, p] = corrcoef(CT_profile, SD_profile, 'rows', 'complete');
        title([num2str(regage(i))  ' days - R: ', num2str(r1(1,2), '%.2f'), ' p: ', num2str(p(1,2))]);
        if i == 1
            legend('SD', 'CT', 'Location', 'best');
        end
        hold off;
    end
end
