%% Script to compare the overlap between hand-drawn ROIs and both UNC and coarse FreeSurfer projected ROIs
% You cannot run this unless you have projected the UNC labels using the
% project_UNC_labels.m script and project_fsaverage_labels.m script
    ...
    for h = 1:length(hemisphere)
        for r = 1:length(roi_list)
            ...
            if ~isempty(gt_data)
                ...
                fprintf('Warning: Empty or missing groundtruth label for %s, %s, %s\n', subj{s}, hemisphere{h}, roi_list{r});
                i_gt = i_gt + 1;
                data_groundtruth{i_gt} = [];
            end
            ...
            if ~isempty(unc_data)
                ...
                fprintf('Warning: Empty or missing UNC label for %s, %s, %s\n', subj{s}, hemisphere{h}, roi_list{r});
                i_unc = i_unc + 1;
                data_UNC{i_unc} = [];
            end
            ...
            if ~isempty(coarse_data)
                ...
                fprintf('Warning: Empty or missing fsaverage projected label for %s, %s, %s\n', subj{s}, hemisphere{h}, roi_list{r});
                i_coarse = i_coarse + 1;
                data_coarse{i_coarse} = [];
            end
        end
    end
end
